...
##############################################################################/
#Linkage disequilibrium in Host populations including resistance markers####
##############################################################################/

onebyhost<-"data/onebyhostALL.txt"
test_LD(onebyhost,"output/onebyhostALL.txt.DIS")
clean_workdir()

#the output file have to be edited so it can be used for representation
LDbyhoAL<-readLines("output/onebyhostALL.txt.DIS")
#removing the first lines that are non relevant
LDcolnames<-unlist(strsplit(LDbyhoAL[13],"\\s+"))
LDbyhoAL<-LDbyhoAL[-c(1:14)]
#for 14 markers and 4 populations there are 
18*17/2*4 #LD tests for markers pairs
LDbyhoAL<-LDbyhoAL[-c(613:length(LDbyhoAL))]
LDbyhoAL<-as.data.table(matrix(unlist(strsplit(LDbyhoAL,"\\s+")),
                               nrow=length(strsplit(LDbyhoAL,"\\s+")),
                               byrow=TRUE))
colnames(LDbyhoAL)<-LDcolnames
LDbyhoAL$`P-Value`<-as.numeric(LDbyhoAL$`P-Value`)

#Linkage disequilibrium in the peach population
LDpeach<-as.matrix(LDbyhoAL[c(1:153),])
row.names(LDpeach)<-paste(LDpeach[,2],LDpeach[,3])
LDpeach<-rbind(LDpeach[,c(2,3,4)],LDpeach[,c(3,2,4)])
LDpeach<-spread(data.frame(LDpeach),2,3,fill="NA",convert=TRUE)
#changing the rownames
...
LDpeach<-LDpeach[,c(-1)]
#reordering the columns and turning the object into a matrix
LDpeach<-as.matrix(LDpeach[c(1,18,2,17,4,10,14,15,16,3,5:9,11:13),
                           c(1,18,2,17,4,10,14,15,16,3,5:9,11:13)])
